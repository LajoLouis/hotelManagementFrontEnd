...
function BookingPayment() {
  ...
  const bookedRooms = bookingCart.map((item) => (
    <div
      key={item.bookingId}
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="md:w-2/3 p-6 flex flex-col justify-between">
          <h1 className="text-3xl font-bold text-gray-800 mb-4">
            <Link to={`/searchedhotel?query=${item.hotel}`}>{item?.hotel}</Link>
          </h1>
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-gray-700">
              <Link to={`/searchedhotel?query=${item.hotel}`}>
                {item?.name}
              </Link>
            </h2>
          ...
          <div className="flex justify-between items-center mb-6">
            <p className="text-lg text-gray-600">{item?.occupants} Adults</p>
          ...
          <div className="flex flex-row text-center justify-between">
            ...
            <button onClick={()=>deletebookedRoom(item.bookingId)}>
